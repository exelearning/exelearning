...
describe('Database Migrations', () => {
    ...
    describe('migrateDown', () => {
        it('should rollback the last migration', async () => {
            ...
            expect(result.success).toBe(true);
            expect(result.rolledBack).toBe('008_project_folders');
        });
        ...
        it('should remove tables after rollback', async () => {
            ...
            // Rollback all 9 migrations to remove all tables
            await migrateDown(db); // rollback 008_project_folders
    ...
    describe('getMigrationStatus', () => {
        ...
        it('should correctly track partial migrations', async () => {
            ...
            // After one rollback, 008_project_folders should be pending
            // All prior migrations are still executed
            expect(status.pending).toContain('008_project_folders');
            expect(status.executed).toContain('001_initial');
    ...
    describe('migration round-trip', () => {
        it('should support migrate up, down, up cycle', async () => {
            ...
            expect(up1.executedMigrations).toContain('008_project_folders');

            // Down - rolls back the last migration (008_project_folders)
            const down = await migrateDown(db);
            expect(down.success).toBe(true);
            expect(down.rolledBack).toBe('008_project_folders');

            // Up again - should re-apply 008_project_folders
            const up2 = await migrateToLatest(db);
            expect(up2.success).toBe(true);
            expect(up2.executedMigrations).toContain('008_project_folders');
        });
    });
