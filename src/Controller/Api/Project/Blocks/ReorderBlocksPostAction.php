...
class ReorderBlocksPostAction extends AbstractController
{
    public function __construct(private readonly BlockService $service)
    {
    }

    #[Route('/api/v2/projects/{projectId}/pages/{pageId}/blocks:reorder', name: 'api_v2_projects_pages_blocks_reorder_post', methods: ['POST'])]
    public function __invoke(string $projectId, string $pageId, Request $request)
    {
        ...
        try {
            $applied = $this->service->reorderBlocks($projectId, $pageId, $order, $step);
