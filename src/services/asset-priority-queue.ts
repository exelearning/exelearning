...
class ServerPriorityQueue {
  ...
  releaseSlot(projectId: string, assetId: string): UploadSlot | undefined {
    ...
    if (idx >= 0) {
      ...
      console.log(
        `[PriorityQueue] Released slot: ${assetId.substring(0, 8)}...`
      );
      return slot;
    }
    return undefined;
  ...
  getStats(
    ...
    return {
      ...
      lowestActivePriority:
        slots.length > 0 ? Math.min(...slots.map((s) => s.priority)) : 0,
    };
  }
  ...
  clearProject(projectId: string): void {
    ...
    console.log(
      `[PriorityQueue] Cleared project: ${projectId.substring(0, 8)}...`
    );
  }
