...
import { createZip as createZipDefault, extractZip as extractZipDefault } from '../services/zip';
import type {
    FileGatorDirRequest,
    FileGatorChangeDirRequest,
    FileGatorCreateRequest,
    FileGatorDeleteItemsRequest,
    FileGatorRenameItemRequest,
    FileGatorCopyMoveItemsRequest,
    FileGatorZipItemsRequest,
    FileGatorUnzipItemRequest,
    FileGatorSaveContentRequest,
    FileGatorGetPathRequest,
    UploadChunkRequest,
    FileListResult,
    FileListItem,
...
const filemanagerSessions = new Map<
    string,
    {
        odeSessionId: string;
        currentPath: string;
...
export function createFilemanagerRoutes(deps: FilemanagerDependencies = {}): Elysia {
     ...
     */
    function getChunkPath(odeSessionId: string, filename: string, identifier: string, chunkNumber: number): string {
     ...
     */
    async function listDirectory(odeSessionId: string, relativePath: string = ''): Promise<FileListResult> {
    ...
    return (
        new Elysia({ prefix: '/filemanager' })
            ...
            .get('/index/:plugin/:sessionId', async ({ params, request, set }) => {
                const { plugin: _plugin, sessionId } = params;
            ...
            .post('/getdir', async ({ body, cookie }) => {
                const data = body as FileGatorDirRequest;
                const dir = data.dir || '';
            ...
            .post('/changedir', async ({ body, cookie }) => {
                const data = body as FileGatorChangeDirRequest;
                const to = data.to || '';
                const sessionId = cookie.odeSessionId?.value;
            ...
            .post('/createnew', async ({ body, cookie }) => {
                const data = body as FileGatorCreateRequest;
            ...
            .post('/deleteitems', async ({ body, cookie }) => {
                const data = body as FileGatorDeleteItemsRequest;
            ...
            .post('/renameitem', async ({ body, cookie }) => {
                const data = body as FileGatorRenameItemRequest;
            ...
            .post('/copyitems', async ({ body, cookie }) => {
                const data = body as FileGatorCopyMoveItemsRequest;
                const items = data.items || [];
                const destination = data.destination || '';
                const sessionId = cookie.odeSessionId?.value;
            ...
            .post('/upload', async ({ body, cookie }) => {
                ...
                try {
                    const data = body as UploadChunkRequest;
                    ...
                    if (file instanceof Blob) {
                        ...
                        buffer = Buffer.from(file as unknown as ArrayBuffer);
                    }

                    // Get filename from request data or file object
                    interface FileWithName extends Blob {
                        name?: string;
                    }
                    const filename = data.resumableFilename || (file as FileWithName).name || 'file';
            ...
            .post('/savecontent', async ({ body, cookie }) => {
                const data = body as FileGatorSaveContentRequest;
            ...
            .post('/getpath', ({ body }) => {
                const data = body as FileGatorGetPathRequest;
            ...
            .post('/zipitems', async ({ body, cookie }) => {
                const data = body as FileGatorZipItemsRequest;
                const items = data.items || [];
                const destination = data.destination || '';
                const name = data.name;
                const sessionId = cookie.odeSessionId?.value;
            ...
            .post('/unzipitem', async ({ body, cookie }) => {
                const data = body as FileGatorUnzipItemRequest;
