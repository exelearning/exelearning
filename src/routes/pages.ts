...
const getAuthMethods = (): string[] => {
    ...
    return methods.split(',').map(m => m.trim().toLowerCase()).filter(m => m);
};
...
export function createPagesRoutes(deps: PagesDependencies = defaultDependencies) {
    ...
    const db = deps.db;
    const { findUserById, findUserByEmail, createUser, findPreference, findProjectByUuid, checkProjectAccess, createProject } =
        deps.queries ?? defaultQueries;
    const { createSession, generateSessionId, getSession } = deps.sessionManager ?? defaultSessionManager;
     ...
     * Pages routes
     */
    return new Elysia({ name: 'pages-routes' })
    .use(cookie())
    .use(jwt({
        name: 'jwt',
        secret: getJwtSecret(),
    ...
    // Login Page
    // =====================================================
    .get('/login', async ({ currentUser, isGuest, cookie, jwt, set, query, request }) => {
        const offline = isOfflineMode();
        const defaultEmail = process.env.DEFAULT_USER_EMAIL || process.env.TEST_USER_EMAIL || 'user@exelearning.net';
        ...
        const rawReturnUrl = (query as any)?.returnUrl || '';
    ...
    // Workarea Page
    // =====================================================
    .get('/workarea', async ({ currentUser, isGuest, query, set, jwt, cookie, request }) => {
            ...
            if (session) {
                ...
                if (project) {
                    ...
                    if (!accessCheck.hasAccess) {
                        console.log(`[Pages] Access denied to project ${projectUuid} for user ${currentUser.id}: ${accessCheck.reason}`);
                    ...
                    console.log(`[Pages] Access denied to in-memory session ${projectUuid}: created by user ${session.userId}, accessed by ${currentUser.id}`);
                    const html = renderTemplate('workarea/access-denied', {
                        basePath,
                        projectId: projectUuid,
        ...
        if (!projectUuid) {
            try {
                ...
                return Response.redirect(prefixPath(`/workarea?project=${newSessionId}`) || `/workarea?project=${newSessionId}`, 302);
            } catch (error) {
        ...
        const isOfflineInstallation = isOfflineMode() ||
...
<html>
  ...
  <body>
    <div id="root">eXeLearning workarea - Template error: ${error.message}</div>
  </body>
</html>`;
