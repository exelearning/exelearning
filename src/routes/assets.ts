...
export function createAssetsRoutes(deps: AssetsDependencies = defaultDependencies) {
    ...
    return new Elysia({ prefix: '/api/projects/:projectId/assets' })
        ...
        try {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                try {
                    ...
                    const existingAsset = await queries.findAssetByClientId(database, fileMeta.clientId, projectIdNum);

                    if (existingAsset) {
                        // Update existing asset
                        await queries.updateAsset(database, existingAsset.id, {
                            storage_path: filePath,
                            file_size: String(fileBuffer.length),
                        });
                        results.push({
 ...
 */
function serializeAsset(asset: Asset): any {
    return {
        id: asset.id,
