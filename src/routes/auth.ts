...
export function createAuthRoutes(deps: AuthDependencies = defaultDeps) {
    ...
    .post('/login_check', async ({ jwt, cookie, body, set, request }) => {
        const email = (body as any)?._username || (body as any)?.email;
        const password = (body as any)?._password || (body as any)?.password;
        ...
        if (!email || !password) {
            ...
            return Response.redirect(`${url.origin}${loginUrl}?error=${encodeURIComponent('Email and password required')}`, 302);
        }

        const user = await findUserByEmail(db, email);
        if (!user) {
            // Redirect back to login with error
            return Response.redirect(`${url.origin}${loginUrl}?error=${encodeURIComponent('Invalid credentials')}`, 302);
        }
    ...
    .get('/logout', async ({ cookie, request, jwtPayload }) => {
        ...
        if (authMethod === 'openid') {
            ...
            if (endSessionEndpoint) {
                ...
                const idTokenCookie = request.headers.get('cookie')
                    ?.split(';')
    ...
    .get('/login/cas/callback', async ({ jwt, cookie, request, query, set }) => {
        ...
        try {
            ...
            const userMatch = body.match(/<cas:user>([^<]+)<\/cas:user>/i) ||
                              body.match(/<user>([^<]+)<\/user>/i);
            ...
            const emailMatch = body.match(/<cas:mail>([^<]+)<\/cas:mail>/i) ||
            ...
            const returnUrlCookie = request.headers.get('cookie')
                ?.split(';')
    ...
    .get('/login/openid/callback', async ({ jwt, cookie, request, query, set }) => {
        ...
        const oidcStateCookie = request.headers.get('cookie')
            ?.split(';')
    ...
    .get('/login/saml', ({ set }) => {
        ...
        return {
            ...
            message: 'SAML authentication is configured but not yet implemented in Elysia. Please use CAS or OpenID Connect.',
        };
    })
