...
class OdeOldXmlWikipediaIdevice
{
    ...
    public static function oldElpWikipediaStructure($odeSessionId, $odePageId, $wikipediaNodes, $generatedIds, $xpathNamespace)
    {
        ...
        foreach ($wikipediaNodes as $wikipediaNode) {
            $fullHtmlView = [];
            ...
            $wikipediaNode->registerXPathNamespace('f', $xpathNamespace);
            // Get blockName
            $blockNameNode = $wikipediaNode->xpath("f:dictionary/f:string[@value='_title']/following-sibling::f:unicode[1]/@value");

            $nodeIdevices = $wikipediaNode->xpath("f:dictionary/f:instance[@class='exe.engine.field.TextAreaField']");
            ...
            foreach ($nodeIdevices as $nodeIdevice) {
                ...
                $fullOdeComponentsSyncHtmlView = '<div class="exe-wikipedia-content">';
                foreach ($fullHtmlView as $htmlView) {
                    $htmlView = str_replace('<p></p>', '', $htmlView);
                    $fullOdeComponentsSyncHtmlView .= $htmlView;
                }
                $fullOdeComponentsSyncHtmlView .= '</div>';
            }
