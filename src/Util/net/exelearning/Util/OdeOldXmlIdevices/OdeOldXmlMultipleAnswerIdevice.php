...
/**
 * OdeOldXmlMultipleAnswerIdevice.
 */
class OdeOldXmlMultipleAnswerIdevice
{
    ...
    public static function oldElpMultipleAnswerStructure($odeSessionId, $odePageId, $caseStudyNodes, $generatedIds, $xpathNamespace)
    {
        ...
        foreach ($caseStudyNodes as $caseStudyNode) {
            ...
            $nodeIdevices = $caseStudyNode->xpath("f:dictionary/f:list/f:instance[@class='exe.engine.field.SelectQuestionField']");

            foreach ($nodeIdevices as $nodeIdevice) {
                $questionAnswers = [];
                $nodeIdevice->registerXPathNamespace('f', $xpathNamespace);
                $nodeIdeviceQuestionTextAreaHtmlContent = $nodeIdevice->xpath("f:dictionary/f:instance/f:dictionary/f:string[@value='content_w_resourcePaths']/
                following-sibling::f:unicode[1]")[0];
                $nodeIdeviceQuestionAnswers = $nodeIdevice->xpath("f:dictionary/f:list/f:instance[@class='exe.engine.field.SelectOptionField']");
                ...
                if (isset($commonReplaces)) {
                    $odeComponentsSyncHtmlView = self::applyReplaces(
                        $commonReplaces,
                        (string) $nodeIdeviceQuestionTextAreaHtmlContent[0]['value']
                    );
                    array_push($fullHtmlView, $odeComponentsSyncHtmlView);
                } else {
                    $odeComponentsSyncHtmlView = (string) $nodeIdeviceQuestionTextAreaHtmlContent[0]['value'];
                    array_push($fullHtmlView, $odeComponentsSyncHtmlView);
                }
