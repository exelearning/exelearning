    ...
    fetchLibraryFiles: async (files: string[]) => {
        ...
        for (const file of files) {
            if (file.startsWith('bootstrap-icons/icons/')) {
                result.set(file, Buffer.from(`<svg data-icon="${file}"></svg>`));
            }
        }
        return result;
    },
...
describe('PrintPreviewExporter', () => {
    ...
    describe('generatePreview', () => {
        ...
        it('should inline bootstrap icon SVGs in print preview HTML', async () => {
            ...
            expect(result.html).not.toContain('libs/bootstrap-icons/icons/lightbulb.svg');
        });
