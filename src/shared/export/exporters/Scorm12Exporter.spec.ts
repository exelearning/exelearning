...
describe('Scorm12Exporter', () => {
    ...
    describe('xAPI emitter scope (ADR-2302-02)', () => {
        it('ships neither the xAPI config nor the emitter loader', () => {
            // SCORM 1.2 carries its own authoritative scoring channel (`cmi.core.score.raw` / `cmi.core.lesson_status`),
            ...
            const first = exporter.generateScormPageHtml(
                samplePages[0],
                samplePages,
            ...
            const second = exporter.generateScormPageHtml(
                samplePages[1],
                samplePages,
                document.getMetadata(),
                false,
