    ...
    renderBlock(
        ...
        const icon =
            ...
            (iconName
                ? iconName.startsWith('bi-')
                    ? { source: 'bootstrap' as const, value: iconName.replace(/^bi-/, '') }
        ...
        if (hasIcon) {
            if (icon.source === 'bootstrap') {
                const iconPath =
                    bootstrapIconDataUris?.get(icon.value || '') ||
                    `${basePath}libs/bootstrap-icons/icons/${icon.value || 'question-circle'}.svg`;
                iconHtml = `<div class="box-icon exe-icon">
<span class="exe-bootstrap-icon" style="--exe-bootstrap-icon-url:url('${this.escapeAttr(iconPath)}');" aria-hidden="true"></span>
</div>
`;
