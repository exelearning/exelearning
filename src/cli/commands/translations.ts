...
import { PINNED_REVISION, VENDORED, VENDORED_ROOT, verifyVendoredTree } from '../../../scripts/vendor-mindmaps';
...
/**
 * Compares the vendored mindmaps tree against what `vendor-mindmaps` would write.
 *
 * The strings live in min/js/script.js, which is generated by building the pinned
 * maintenance-fork revision -- the sources are in exelearning/mindmaps, not here. The
 * tree is committed, so a fresh checkout has it, but a hand-edited or half-regenerated
 * bundle scans without complaint while the strings it is missing look obsolete. Deleting
 * the bundle and running --remove-obsolete destroys four mindmaps trans-units in every
 * locale, translations included, which is what this guards.
 *
 * Needs no network and no build: every expected hash is recorded in the vendor script.
 */
function inspectMindmapsTree(cwd: string): { complete: boolean; detail: string } | null {
    const drift = verifyVendoredTree(path.join(cwd, VENDORED_ROOT));

    // Nothing the manifest expects is present, so this is not a vendored tree at all and
    // there is nothing to compare against -- the same case the edicuatex inspection
    // reports as unknown when its package is not installed. Directory existence, checked
    // by the caller, is the only claim that can be made here.
    if (drift.missing.length === VENDORED.length) {
        return null;
    }

    // `extra` is not a trust problem: a leftover file can only add keys, never hide one.
    const unusable = [...drift.missing, ...drift.changed].sort();
    if (unusable.length === 0) {
        return { complete: true, detail: `in sync with the pinned fork revision ${PINNED_REVISION.slice(0, 10)}` };
    }
...
export const GENERATED_SOURCE_DIRS: GeneratedSource[] = [
    ...
    {
        path: 'public/app/common/mindmaps',
        regenerateWith: 'make vendor-mindmaps',
        reason: 'holds the mindmaps interface strings, generated from the pinned exelearning/mindmaps revision',
        inspect: inspectMindmapsTree,
    },
];
...
export async function extractTranslationKeys(): Promise<Set<string>> {
    ...
    const quotedPatterns = [
        ...
        /(?<![.\w])_\(\s*'((?:[^'\\]|\\.)*)'/g, // _('key')
        /(?<![.\w])_\(\s*"((?:[^"\\]|\\.)*)"/g, // _("key")
        // _r() is the mindmaps name for its longer help texts. eXeLearning has one GUI
        // translator and langs/all.js points both names at it, so the catalogue should
        // care about the string rather than which alias called it. Without these two the
        // eight reachable help strings are invisible and can never be translated.
        /(?<![.\w])_r\(\s*'((?:[^'\\]|\\.)*)'/g, // _r('key')
        /(?<![.\w])_r\(\s*"((?:[^"\\]|\\.)*)"/g, // _r("key")
        /\bc_\(\s*'((?:[^'\\]|\\.)*)'/g, // c_('key')
        /\bc_\(\s*"((?:[^"\\]|\\.)*)"/g, // c_("key")
