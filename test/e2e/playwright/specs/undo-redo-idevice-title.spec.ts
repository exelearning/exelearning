...
test.describe('Undo/Redo iDevice Icon - Issue #956', () => {
    ...
    test('should visually update block icon after redo without page reload', async ({
        ...
        await page.waitForFunction(
            () => {
                ...
                if (!iconBtn) return false;
                const hasImg = iconBtn.querySelector('img') !== null;
                const hasBootstrapIcon = iconBtn.querySelector('.exe-bootstrap-icon') !== null;
                return hasImg || hasBootstrapIcon;
            },
            undefined,
            { timeout: 10000 },
        );
    ...
    test('should handle multiple icon change undo/redo cycles correctly', async ({
        ...
        const iconMatchScript = (expectedSrc: string) => {
            ...
            const bootstrapIcon = iconBtn.querySelector('.exe-bootstrap-icon');
            if (!bootstrapIcon) return false;

            const computed = window.getComputedStyle(bootstrapIcon);
            const src =
                bootstrapIcon.style.webkitMaskImage ||
                bootstrapIcon.style.maskImage ||
                bootstrapIcon.style.backgroundImage ||
