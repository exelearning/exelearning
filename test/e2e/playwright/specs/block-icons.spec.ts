    ...
    test('should display icons correctly in the block icon selection modal', async ({
        ...
        if (iconCount > 0) {
            ...
            for (let i = 0; i < sampleCount; i++) {
                ...
                // Theme/custom icons render as img; bootstrap modal icons render via sprite SVG.
                ...
                const bootstrapSprite = icon.locator('.exe-bootstrap-icon-sprite use');
                await expect(bootstrapSprite).toHaveCount(1);
                const href = await bootstrapSprite.getAttribute('href');
                expect(href).toBeTruthy();
                expect(href).toContain('/libs/bootstrap-icons/bootstrap-icons.svg#');
            }
        }
