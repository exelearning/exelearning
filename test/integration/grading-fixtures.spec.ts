 ...
 * scorm2004, ims, epub3) and include `pageCount` in the config. Builds without it
 * inject the config only from `Html5Exporter` / `PageExporter`, with the identity
 * keys `{odeId, packageTitle, language}` and no `pageCount` — a SCORM package then
 * ships the emitter script but no config, so the emitter falls back to the served
 * URL for its base IRI.
 *
        ...
        it('injects the xAPI identity config on every page', () => {
            for (const page of pkg.htmlPages) {
                ...
                if (BUILD_SHIPS_XAPI_CONFIG_EVERYWHERE) {
                    expect(cfg.pageCount).toBe(2);
                } else {
                    // This build's exporters inject identity keys only.
                    expect(cfg.pageCount).toBeUndefined();
                }
            }
        });
