...
describe('ELPX Download Integration', () => {
    ...
    describe('HTML5 Export with download-source-file iDevice', () => {
        ...
        it('should include fflate script tag in HTML head (via library detection)', async () => {
            await exporter.export();
            const indexHtml = zip.getFileAsString('index.html');
            expect(indexHtml).toBeDefined();

            // fflate should be loaded before the download script
            // The library detector adds these based on class patterns
            // Note: This depends on LibraryDetector detecting the pattern
        });
    });
