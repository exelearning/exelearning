...
The workarea preview renders **untrusted authored content** in an **opaque-origin** sandboxed iframe (`sandbox="allow-scripts allow-popups allow-forms"`, no `allow-same-origin`), same-origin, no subdomain. `Html5Exporter.generateForPreview()` produces the files (so the preview exactly matches the exported output) and a deterministically-selected transport **provider** delivers them. Two transports remain (`srcdoc` was removed, [ADR-0015](architecture/adr/ADR-0015-opaque-http-preview-in-privileged-contexts-and-trusted-static-service-worker.md)): **`HttpPreviewProvider`** — an ephemeral **opaque** HTTP capability session used by the server, by Electron (`app://localhost`), and by embedded editors whose host injects a `previewHttp` block (embedded **fails closed** without it); and **`StaticServiceWorkerPreviewProvider`** — the same-origin Service Worker below, used **only** by standalone static/PWA builds as a **trusted-content compatibility mode** (`opaqueSafe = false`, **not** a security boundary; a Service Worker cannot serve an opaque iframe). See **[doc/development/preview-architecture.md](development/preview-architecture.md)** for the full design and the browser evidence. The Service-Worker description that follows documents that standalone static/PWA transport.
