...
| `teacherOnly` | boolean string | Restrict block to teacher view (hidden by default in exports; revealed with `?exe-teacher=1`) |
...
| `teacherOnly` | boolean string | Restrict iDevice to teacher view (hidden by default in exports; revealed with `?exe-teacher=1`) |
| `cssClass` | string | Optional extra CSS class(es) |
