...
related:
  ...
  prs: [2199]
  sdds: []
  adrs: [ADR-0002, ADR-0006, ADR-0016]
supersedes: []
superseded_by: []
ai_assistance:
  tool: "Claude Code"
...
# ADR-0017: The in-content embed shim stays inert until a host completes the handshake
...
The alternative — the shipped behaviour before this — was to activate on "framed and
opaque". It looks equivalent and is not: **`file://` is an opaque origin in every engine**,
so a framed local package satisfied both conditions, promoted its embeds to placeholders,
and then waited forever for a host that did not exist. The reader was left with permanent
black boxes in content that had already been delivered.

That failure mode is not compensated for here, it is **designed out**: promotion is gated
on an addressed `welcome` from a host that resolved this exact window. A broadcast
`request` deliberately cannot unlock it, because a broadcast reaches windows we do not
host.

Evidence in both directions, in three engines: `external-media-artifacts.spec.ts` asserts
that a hosted document promotes its embeds *and* that an unhosted one is left exactly as
authored. Both run against the built artifacts, not the sources.

Accepted because the failure it prevents is silent, permanent, and reaches content already
in circulation — the worst combination of the three.
...
- The shim cannot know its host by origin — in an opaque origin `event.origin` is the
  string `"null"`, so window identity is the only usable anchor (ADR-0006).
...
- [ADR-0006](ADR-0006-render-editor-preview-in-an-opaque-origin-sandbox.md) — why
  `event.origin` is unusable and window identity is the anchor
- [ADR-0002](ADR-0002-hybrid-preview-trust-boundary.md) — the trust boundary this serves
