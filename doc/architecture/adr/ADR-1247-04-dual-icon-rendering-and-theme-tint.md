...
- **Picker inlines the glyph from the in-memory sprite**: in the modal, Material options render via `renderMaterialInlineIcon` as a self-contained `<svg class="exe-material-icon-sprite" viewBox="…" fill="currentColor"><path …/></svg>` built from the sprite `loadMaterialSprite` parsed at startup, while applied/content icons use the mask `data:` URI (`renderMaterialMaskIcon`) — `public/app/common/blockIconRuntime.js`, `public/app/workarea/project/idevices/content/blockNode.js`, styled in `_modals.scss`. PR #1497 originally rendered each option as an external sprite reference (`<use href="…/material-icons.svg#<name>">`). Under Electron's custom `app://` scheme Chromium does not share the external SVG document between `<use>` elements, so the ~3 800-option picker fetched and parsed the 1.6 MB sprite once per option and froze the renderer (issue #2419, fixed in PR #2420). The picker must never reference the sprite file per option; before the sprite is loaded it falls back to the mask placeholder that `loadMaterialSprite` hydrates.
...
- The picker uses inline `<svg>` glyphs for browsing (cheapest to lay out by the thousand, no image decode, no per-option fetch) while applied icons use the mask `data:` URI — two Material variants coexist for different jobs, both fed from the single sprite parsed once in memory.
