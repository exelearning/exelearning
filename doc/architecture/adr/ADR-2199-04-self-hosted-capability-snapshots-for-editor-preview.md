...
related:
  prs: [1968]
  changes: []
  adrs: [ADR-2199-02, ADR-2199-05, ADR-2199-06]
supersedes: []
superseded_by: []
ai_assistance:
  tool: "Claude Code"
...
# ADR-2199-04: Self-hosted capability snapshots for the editor preview (minimal subset)
...
(ADR-2199-02). The embedded-host case (ADR-2199-01) already had a client for exactly
...
powerful but is the operational surface ADR-2199-02 deliberately rejects for the
ordinary editor.
...
- **Serving route** (`GET /preview-snapshot/:id/:path*`, **no auth**): the
  ...
  `Referrer-Policy`, and — per [ADR-2199-06](ADR-2199-06-emit-sandbox-csp-on-every-scriptable-document-type.md)
  — a `sandbox`-first CSP on **every** scriptable type (HTML, XHTML, SVG, XML,
  PDF), so a directly opened capability URL stays opaque
  ([ADR-2199-05](ADR-2199-05-render-editor-preview-in-an-opaque-origin-sandbox.md)).
...
- **Absent where impossible:** the routes are registered only by the Bun server.
  ...
  resolution never offers it (ADR-2199-02's no-silent-fallback rule).
...
- [ADR-2199-02](ADR-2199-02-hybrid-preview-trust-boundary.md), [ADR-2199-05](ADR-2199-05-render-editor-preview-in-an-opaque-origin-sandbox.md), [ADR-2199-06](ADR-2199-06-emit-sandbox-csp-on-every-scriptable-document-type.md)
