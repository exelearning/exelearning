...
related:
  ...
  prs: [2199]
  sdds: []
  adrs: [ADR-0017, ADR-0020, ADR-0022, ADR-0023]
supersedes: []
superseded_by: []
ai_assistance:
  tool: "Claude Code"
...
ADR-0023 establishes that **no provider player survives an opaque origin**, and the
...
trusted page — and there ADR-0022 already answers it: no third-party JavaScript on the
...
- **Load the provider SDK instead of raw messages.** Rejected on measurement: blocked by the
  content CSP, and unusable from a null origin even if it loaded. See ADR-0022.
- **Keep the local player and relax the sandbox for packages with video.** Rejected outright:
  ...
  (ADR-0023 uses it only as a measurement control).
