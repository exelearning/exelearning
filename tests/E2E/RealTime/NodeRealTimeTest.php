...
final class NodeRealTimeTest extends BaseE2ETestCase
{
    ...
    public function test_nodes_changes_propagate_between_two_clients(): void
    {
        ...
        (new Node($a1Title, $workareaB, $a1->getId(), $rootB))->assertVisible($a1Title, 120, true);

        // A renames the node
        $a1Renamed = $a1Title . ' (renamed)';
        $a1->rename($a1Renamed);
        // B sees rename
        (new Node($a1Renamed, $workareaB, $a1->getId(), $rootB))->assertVisible($a1Renamed, 120, true);
        (new Node($a1Title, $workareaB, null, $rootB))->assertNotVisible($a1Title, 120, true);
        ...
        (new Node($a2Title, $workareaB, $a2->getId(), $rootB))->assertVisible($a2Title, 120, true);
        $a2->delete();
        (new Node($a2Title, $workareaB, $a2->getId(), $rootB))->assertNotVisible($a2Title, 120, true);
        ...
        (new Node($b1Title, $workareaA, $b1->getId(), $rootA))->assertVisible($b1Title, 120, true);

        // B renames
        $b1Renamed = $b1Title . ' (renamed)';
        $b1->rename($b1Renamed);
        // A sees rename
        (new Node($b1Renamed, $workareaA, $b1->getId(), $rootA))->assertVisible($b1Renamed, 120, true);
        (new Node($b1Title, $workareaA, null, $rootA))->assertNotVisible($b1Title, 120, true);

        // B deletes
        $b1->delete();
        (new Node($b1Renamed, $workareaA, $b1->getId(), $rootA))->assertNotVisible($b1Renamed, 120, true);
