...
**Key Components:**
- `public/app/preview-sw.js` - Service Worker that intercepts `/viewer/*` requests
- `public/app/workarea/interface/elements/previewPanel.js` - Preview panel UI
- `src/shared/export/exporters/Html5Exporter.ts` - Generates export files (also used for preview via `generateForPreview()`)

**How it works:**
1. When preview is requested, `Html5Exporter.generateForPreview()` generates all export files in memory (as `Map<string, Uint8Array | string>`)
2. Files are sent to the Service Worker via `postMessage({ type: 'SET_CONTENT', files })`
3. The iframe loads `/viewer/index.html` - all requests are intercepted by the SW
4. SW serves files from its in-memory cache with correct MIME types

**Benefits:**
- Preview **exactly matches** HTML export output (same code path)
- No blob:// URL workarounds needed
- PDFs work with native browser viewer (not in nested blob context)
- External links handled correctly
- Multi-page navigation works like real export

**Key files:**
```
public/app/preview-sw.js          # Service Worker
public/app/workarea/interface/elements/previewPanel.js  # Preview panel
src/shared/export/exporters/Html5Exporter.ts           # Export generator
src/shared/export/browser/index.ts                      # Browser export API
```
...
**Preview Page Elements (inside iframe):**
```javascript
// Preview iframe
'#preview-iframe'
// Inside the iframe (use frameLocator):
...
The preview opens in a **side panel** (iframe served by Service Worker):
...
// Access iframe content (Service Worker serves /viewer/*)
const iframe = page.frameLocator('#preview-iframe');
await iframe.locator('article').first().waitFor({ state: 'attached', timeout: 10000 });
```
...
**6. Open Preview Panel (Service Worker based):**
```javascript
// Click preview button to open side panel
await page.click('#head-bottom-preview');
const previewPanel = page.locator('#previewsidenav');
...
// Access preview iframe content (served via Service Worker at /viewer/*)
const iframe = page.frameLocator('#preview-iframe');

// Wait for preview content to load (multi-page HTML, not SPA)
await iframe.locator('article').first().waitFor({ state: 'attached', timeout: 10000 });
...
**9. Verify PDF/Media in Preview (Service Worker based):**
```javascript
// Open preview panel
await page.click('#head-bottom-preview');
await page.locator('#previewsidenav').waitFor({ state: 'visible', timeout: 15000 });

// Wait for Service Worker to load content
await page.waitForTimeout(2000);

// Preview uses Service Worker - content is served via /viewer/* URLs
// PDFs use native browser viewer (no PDF.js workaround needed)
const iframe = page.frameLocator('#preview-iframe');

// Verify media element is present
const mediaElement = iframe.locator('video, audio, embed, object').first();
await expect(mediaElement).toBeVisible({ timeout: 10000 });
```

### Preview Navigation (Multi-Page via Service Worker)

The preview is served by a Service Worker as multi-page HTML (same as export output):
- Main page: `/viewer/index.html`
- Sub-pages: `/viewer/html/{page-id}.html`
- Navigation via standard links (not SPA anchors)
- Service Worker intercepts all `/viewer/*` requests and serves from memory
...
```javascript
{
  "pageId": {
    name: "Page Title",
    fileName: "html/page-id.html",  // Relative path for multi-page navigation
    fileUrl: "html/page-id.html",
    blocks: { ... }
  }
}
```
