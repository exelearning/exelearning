...
var $exeDevice = {
  ...
  init: function (element, previousData, path) {
    ...
    var html = '\
      <div class="exe-info">'+ _("Complete the table to define a scoring guide. Define the score or value of each descriptor.") + ' <a href="https://youtu.be/T_QtGkH68EY?t=92" target="_blank" hreflang="es" rel="lightbox">' + _("Learn how to apply a rubric") + '</a>.</div>\
  ...
  resetForm: function (createEditor) {
    ...
    var html = '\
      <p>\
        ...
        <span id="ri_NewTableOptions">\
          <label for="ri_NewTable" class="visually-hidden">'+ _("New rubric: ") + '</label>\
    ...
    if (typeof ($exeDevice.cedecRubrics) == 'undefined') {
      ...
      $("#ri_LoadCEDECRubrics").click(function () {
        ...
        $.ajax({
          url: $exeDevice.idevicePath+"cedec.json?version"+timestamp,
          dataType: 'json',
  ...
  jsonToTable: function (data, mode) {
    ...
    if (mode == "normal") {
      ...
      if (instructions != "") intro = '<p class="exe-rubrics-instructions">' + instructions + '</p>';
      ...
      if (author != "" || authorURL != "" || license != "") {
        var info = '<p class="exe-rubrics-authorship' + visibility + '">';
        if (author != "") {
        ...
        if (license != "") {
          ...
          if (license.indexOf("CC") == 0) info += '<a href="https://creativecommons.org/licenses/" rel="license nofollow noopener" target="_blank" title="Creative Commons ' + license + '">' + license.replace("CC-", "CC ") + '</a>'
      ...
      for (var i in $exeDevice.ci18n) {
        lang += '<li class="' + i + '">' + $exeDevice.ci18n[i] + '</li>';
      }
      lang += '</ul>';
    ...
    $("#ri_Reset").click(function () {
      eXe.app.confirm(_("Attention"), _("Revert all changes? This can't be undone."), function () {
