...
function walkAreas(dataset) {
    ...
    for (const [etapa, niveles] of Object.entries(dataset)) {
        if (SKIP.includes(etapa)) continue;
...
describe('lomloe-ES-GA.json (Galicia concretion)', () => {
    ...
    it('contains a Config.CC_DESCRIPTIONS object', () => {
        expect(data.Config).toBeDefined();
        expect(data.Config.CC_DESCRIPTIONS).toBeDefined();
    });

    it('Config.CC_DESCRIPTIONS contains expected competencia clave keys', () => {
        expect(data.Config.CC_DESCRIPTIONS['CC1']).toBeDefined();
        expect(data.Config.CC_DESCRIPTIONS['STEM2']).toBeDefined();
    });

    it('Config is not iterated as an etapa by walkAreas', () => {
        const areas = walkAreas(data);
        for (const { etapa } of areas) {
            expect(etapa).not.toBe('Config');
        }
    });
});
