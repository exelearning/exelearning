...
- **Primària first cicle (1r i 2n) — sabers-only**: the currently in-force **Decret 106/2022** does **not** publish `criteris d'avaluació` for the first cicle. Its Annex III criteris tables only carry `2n cicle (4t curs)` and `3r cicle (6é curs)` columns, whereas the *sabers bàsics* matrix does include a `1r cicle` column. The dataset represents this faithfully: `1r`/`2n d'Educació Primària` carry sabers bàsics but no criteris d'avaluació. (The state RD 157/2022 does define first-cicle criteris, and a 2026 draft modification of Decret 106/2022 adds a `1r cicle` criteris column, but it is not part of the published decree.)
...
- **Known gaps (ESO, pending a follow-up extraction pass):** several ESO subjects (e.g. Matemàtiques, Geografia i Història, Educació Física, Tecnologia) currently have no *sabers bàsics* because their sabers are laid out by "sentits"/blocs the parser does not yet read, and *Valencià: Llengua i Literatura* is not yet extracted. Their criteris d'avaluació are present and correct.
