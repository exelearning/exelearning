...
var iAdmin = {
    ...
    video: {
        ...
        enable: function (url, type) {
            ...
            if (type == 'mediateca') {
                ...
                $("#player").html(`<video width="448" height="356" class="mejs__player" data-mejsoptions='{"pluginPath": "${iAdmin.domainPath}/app/common/mediaelement/", "alwaysShowControls": "true"}'><source src="${iAdmin.domainPath}/${url}" /></video>`);
    ...
    getBlockInstructions: function (block) {
        ...
        timeInstr = timeInstr.replace("%s", `<img src="${iAdmin.domainPath}/files/perm/idevices/base/interactive-video/edition/editor/images/timer.png" width="24" height="24" alt="` + $i18n.Current_Time + '" />');
    ...
    save: function (type) {
        ...
        if (iAdmin.globals.warnWhenSave) {
            iAdmin.msg.txt('<p><strong>' + $i18n.Important + ':</strong></p><p>' + $i18n.Click_On_Save_Warning + `</p><p><img src="${iAdmin.domainPath}/files/perm/idevices/base/interactive-video/edition/editor/images/help/save_warning.png" width="48" height="48" alt="` + $i18n.Save + '" /></p><p><a href="#" onclick="iAdmin.globals.warnWhenSave=false;iAdmin.msg.hide();return false">' + $i18n.Hide_Warning + '</a></p>');
        }
            ...
            tinymce.init({
                ...
                file_picker_callback: function (cb, value, meta) {
                    ...
                    input.onchange = function () {
                        ...
                        fd.append('odeSessionId', [top.eXeLearning.app.project.odeSession]);
                        ...
                        top.eXe.app.uploadLargeFile(fd).then(response => {
                            if (response && response.savedPath && response.savedFilename) {
                                ...
                                cb(fullPath, { title: response.savedFilename, size: response.savedFileSize });
                            } else {
