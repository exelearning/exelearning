...
var $exeDevice = {
  ...
  completeLicense: function (str) {
    ...
    for (let i = 0; i < licenses.length; i++) {
      ...
      if (license[0] === str) {
        ...
        css = license[1].split("/");
        css = "cc cc-" + css[0];
        str = '<a href="https://creativecommons.org/licenses/' + license[1] +
          '/" rel="license" class="' + css + '"><span></span>Creative Commons ' + type + '</a>';
      }
    }
    return str;
  },
  ...
  createForm: function () {
    ...
    if (data1 == '-' && data2 == '-' && data3 == '-' && data4 == '-') {
      let text = _("Please don't forget to check the Properties tab: Title, language, license, author, description...");
      eXe.app.alert(text);
    }
  ...
  save: function () {
    ...
    if (dpiButtonBGcolor != '' && dpiButtonBGcolor.length == 7) css += 'background-color:' + dpiButtonBGcolor + ';'
    var dpiButtonTextColor = $("#dpiButtonTextColor").val();
    if (dpiButtonTextColor != '' && dpiButtonTextColor.length == 7) css += 'color:' + dpiButtonTextColor + ';'
    ...
    html += '<a download="exe-package:elp-name" href="exe-package:elp"' + css + '>' + dpiButtonText + '</a>';
    html += '</p>';
