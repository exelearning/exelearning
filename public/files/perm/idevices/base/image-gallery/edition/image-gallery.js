var $exeDevice = {
  ...
  attributionData: {},
  attributionDataKeys: ["title", "linktitle", "author", "linkauthor", "license"],
  ...
  createForm: function () {
    ...
    this.idImage = this.ideviceBody.querySelectorAll(".imgSelectContainer").length;
  },
  ...
  loadPreviousValues: function () {
    ...
    let incrementalId = 0;
    if (Object.keys(data).length > 1) { this.ideviceBody.querySelector("#textMsxHide").style.display = "none"; }
    // Load images
   ...
   */
  save: function () {
    let divImages = this.ideviceBody.querySelectorAll(".imgSelectContainer");
    ...
    this.dataIds.forEach(element => {
      let thumbnailURL = this.ideviceBody.querySelector(`#${element}`).getAttribute("src");
      let imageURL = this.ideviceBody.querySelector(`#${element}`).getAttribute('origin');
      let imageTitle, imageLinkTitle, imageAuthor, imageLinkAuthor, imageLicense;
      imageTitle = imageLinkTitle = imageAuthor = imageLinkAuthor = imageLicense = "";
      ...
      if ($exeDevice.attributionData[element]) {
        ...
        imageLinkAuthor = $exeDevice.attributionData[element].linkauthor;
        imageLicense = this.getLicenseValue($exeDevice.attributionData[element].license);
      }
  ...
  addImageButtonBehaviour: function () {
    this.ideviceBody.querySelector("#addImageButton").addEventListener("click", (event, id) => {
  ...
  readFile: function (file) {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (field) => { resolve(field.target.result); };
  ...
  processFile: async function (file) {
    try {
      ...
      let images = $exeDevice.ideviceBody.querySelectorAll(".imgSelectContainer");
  ...
  addUploadImage: async function (imageData, imageName) {
    ...
    if (response && response.savedPath && response.savedFilename && response.savedThumbnailName) {
  ...
  addImageBehaviour: async function () {
    this.ideviceBody.querySelector("#imageLoaded").addEventListener("change", field => {
  ...
  addImageHTML: function (id, originURL, thumbnailURL) {
    ...
    this.ideviceBody.querySelector("#imagesContainer").appendChild(imgContainer);
  ...
  createAttributionBodyHTML: function (id) {
    ...
    let attrBodyHTML = '<div class="form-properties"><div class="properties-body-container"><div class="exe-properties-form-content"><div class="exe-table-content">';
    attrBodyHTML += this.createInputHTML(`${imgId}_${this.titleId}`, this.titleText, "", attrTitle);
    attrBodyHTML += this.createInputHTML(`${imgId}_${this.linktitleId}`, this.linktitleText, "", attrLinkTitle);
    attrBodyHTML += this.createInputHTML(`${imgId}_${this.authorId}`, this.authorText, "", attrAuthor);
    attrBodyHTML += this.createInputHTML(`${imgId}_${this.linkauthorId}`, this.linkauthorText, "", attrLinkAuthor);
    attrBodyHTML += this.createDataListHTML(`${imgId}_${this.licenseId}`, this.licenseText, this.licensePlacecholder, this.licenseOptions, attrLicense);
  ...
  addEventButtonsBehaviour: function (id) {
    let imageElement = this.ideviceBody.querySelector(`#imgSelectContainer_${id}`);
    // Add button
    /*
    ...
    */
    // Attribution button
    this.ideviceBody.querySelector(`#buttonAttribution_${id}`).addEventListener("click", () => {
      ...
      eXeLearning.app.modals.confirm.show({
        ...
        confirmExec: () => {
          ...
          $exeDevice.attributionData[imgId] = {
            title: $(`#${imgId}_${$exeDevice.titleId}`)[0].value,
            linktitle: $(`#${imgId}_${$exeDevice.linktitleId}`)[0].value,
            author: $(`#${imgId}_${$exeDevice.authorId}`)[0].value,
            linkauthor: $(`#${imgId}_${$exeDevice.linkauthorId}`)[0].value,
            license: $(`#${imgId}_${$exeDevice.licenseId}-options`)[0].previousElementSibling.value
          };
        }
      });
    });
    // Modify button
    this.ideviceBody.querySelector(`#buttonModify_${id}`).addEventListener("click", () => {
      this.editionId = id;
      this.ideviceBody.querySelector("#addImageButton").click();
    });
    // Remove button
    this.ideviceBody.querySelector(`#buttonRemove_${id}`).addEventListener("click", () => {
      imageElement.remove();
      this.disabledButtons();
      if (this.checkImages() == 0) $exeDevice.ideviceBody.querySelector("#textMsxHide").style.display = "";
      $exeDevice.getReferences();
    });
    // Move up button
    this.ideviceBody.querySelector(`#buttonUp_${id}`).addEventListener("click", () => {
    ...
    // Move down button
    this.ideviceBody.querySelector(`#buttonDown_${id}`).addEventListener("click", () => {
  ...
  addDragAndDropBehaviour: function () {
    ...
    const dropArea = $exeDevice.ideviceBody.querySelector(".imageGalleryIdeviceForm");
  ...
  addSortableBehaviour: function (item) {
    ...
    let sortables = $exeDevice.ideviceBody.querySelectorAll(".imgSelectContainer");
  ...
  dropAreaEvents: function (dropAreaElement) {
    ["dragenter", "dragover", "dragleave", "drop"].forEach(eventName => {
      dropAreaElement.addEventListener(eventName, this.preventDefaults, false);
    });

    ["dragenter", "dragover"].forEach(eventName => {
      dropAreaElement.addEventListener(eventName, this.dropAreaHighlight, false);
    });

    ["dragleave", "drop"].forEach(eventName => {
      dropAreaElement.addEventListener(eventName, this.dropAreaUnhighlight, false);
    });

    dropAreaElement.addEventListener("drop", this.dropAreaHandleDrop, false);
  },
  ...
  dropAreaHighlight: function (e) {
    ...
    if (itemDragging === null) {
      ...
      $exeDevice.areasBeforeFirstItems.forEach((area, index) => {
        if (e.pageX < area.right && e.pageX > area.left && e.pageY > area.top && e.pageY < area.bottom) {
          ...
          if (itemDragging.getBoundingClientRect().top >= area.top) {
            ...
            $exeDevice.firstItems[index].previousElementSibling.classList.add("highlight");
          }
        }
      })
      $exeDevice.areasAfterLastItems.forEach((area, index) => {
        if (e.pageX < area.right && e.pageX > area.left && e.pageY > area.top && e.pageY < area.bottom) {
          ...
          if (itemDragging.getBoundingClientRect().top <= area.top) {
            ...
            $exeDevice.lastItems[index].nextElementSibling.classList.add("highlight");
          }
        }
      })
    }
  ...
  itemHighlight: function (e) {
    ...
    let itemDragging = $exeDevice.ideviceBody.querySelector(".dragging");
    let imgContainers = $exeDevice.ideviceBody.querySelectorAll(".imgSelectContainer");
    ...
    let middleReference = imgContainer.getBoundingClientRect().left + imgContainer.getBoundingClientRect().width / 2;
    if ($exeDevice.indexDraggingItem < $exeDevice.indexOverItem) {
      if (e.pageX < middleReference) {
        $exeDevice.imgContainerHighlighted = imgContainers[$exeDevice.indexOverItem - 1];
        imgContainers[$exeDevice.indexOverItem - 1].classList.add("highlight");
      }
      else {
  ...
  dropAreaHandleDrop: function (e) {
    $exeDevice.ideviceBody.querySelector("#textMsxHide").style.display = "none";
  ...
  itemsDropAreaEvents: function (itemDropArea) {
    ...
    ["dragenter", "dragover", "dragleave", "drop"].forEach(eventName => {
      itemDropArea.addEventListener(eventName, this.preventDefaults, false);
    });
    ["dragenter", "dragover"].forEach(eventName => {
      itemDropArea.addEventListener(eventName, this.dropAreaHighlight, false);
      itemDropArea.addEventListener(eventName, this.itemHighlight, false);
    });
    ["drop"].forEach(eventName => {
      itemDropArea.addEventListener(eventName, this.itemUnhighlight, false);
    });
    itemDropArea.addEventListener("drop", this.itemsDropAreaHandleDrop, false);
  ...
  getReferences: function () {
    let dropAreaDataPosition = $exeDevice.ideviceBody.querySelector(".imageGalleryIdeviceForm").getBoundingClientRect();
    let sortables = $exeDevice.ideviceBody.querySelectorAll(".imgSelectContainer");
    let rowsData = {};
