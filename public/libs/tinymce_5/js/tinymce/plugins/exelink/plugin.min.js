...
(function () {
    ...
    var extractFromAnchor = function (editor, anchor) {
        ...
        if (title != Optional.none()) {
            actualSize = dom.getAttrib(anchor, 'size');

            let fileInfoSpan = anchor && anchor.nextElementSibling && anchor.nextElementSibling.classList.contains('exe-link-data') ? anchor.nextElementSibling : null;
            if ((!actualSize || actualSize === '') && fileInfoSpan) {
                let spanText = fileInfoSpan.textContent || '';
                let match = spanText.match(/(\d+\.?\d*)\s*(B|KB|MB)/);
                if (match) {
                    actualSize = match[1] + ' ' + match[2];
                }
            }
            showFileInfo = fileInfoSpan !== null;
        } else {
            showFileInfo = false;
        }

        if (url != Optional.none()) {
            let fullOnClick = dom.getAttrib(anchor, 'onclick', '');
            ...
            //Events values
            actualOnFocus = dom.getAttrib(anchor, 'onfocus', '');
            actualOnBlur = dom.getAttrib(anchor, 'onblur', '');
            actualOnDBLClick = dom.getAttrib(anchor, 'ondblclick', '');
            actualOnMouseOver = dom.getAttrib(anchor, 'onmouseover', '');
            actualOnMouseMove = dom.getAttrib(anchor, 'onmousemove', '');
            actualOnMouseOut = dom.getAttrib(anchor, 'onmouseout', '');
        }
    ...
    var handleSubmit = function (editor, info) {
        return function (api) {
            ...
            let baseTitle = api.getData().title.replace(/\s*\[\S+ - [\d.]+ (?:B|KB|MB)\]$/, '');
            let newTitle = baseTitle + ' ' + addToTitle;
            ...
            for (var i = 0; i < pElem.length; i++) {
                var pContent = pElem[i];
                if (pContent.querySelector('iframe, video, audio, embed, object, .mce-preview-object, .mce-object-iframe, .mce-object-video, .mce-object-audio')) {
                    continue; // Don't remove paragraphs with media content
                }
                if (pContent.textContent.trim() === '' && pContent.childNodes.length > 0 && pContent.childNodes[0].nodeName === 'SPAN') {
                    var spanChild = pContent.childNodes[0];
                    if (spanChild.classList && (
                        spanChild.classList.contains('mce-preview-object') ||
                        spanChild.classList.contains('mce-object-iframe') ||
                        spanChild.classList.contains('mce-object-video') ||
                        spanChild.classList.contains('mce-object-audio') ||
                        spanChild.hasAttribute('data-mce-object')
                    )) {
                        continue; // Don't remove media placeholders
                    }
                    editor.dom.setOuterHTML(pContent, '<p><br data-mce-bogus="1"></p>');
                }
            }
            ...
            setTimeout(() => {
                ...
                let span = linkToChange.nextElementSibling && linkToChange.nextElementSibling.classList.contains('exe-link-data') ? linkToChange.nextElementSibling : null;

                if (showFileInfo && fileSize != '') {
                    //Add or update file info
                    linkToChange.title = newTitle;
                    let newSpanHTML = '<span class="exe-link-data file-size">(' + fileExtension
                        + ' - ' + fileSize + ' <abbr title=' + GetUnitText(fileUnit) + '>' + fileUnit + '</abbr>)</span>';
                    if (span != null) {
                        //Change info file data
                        editor.dom.setOuterHTML(span, newSpanHTML);
                    } else {
                        //Insert span after link
                        linkToChange.insertAdjacentHTML('afterend', newSpanHTML);
                    }
                } else {
                    //Remove span if present
                    linkToChange.title = api.getData().title;
                    if (span) editor.dom.setOuterHTML(span, '');
                }
    ...
    var getInitialData = function (info, defaultTarget, api) {
        ...
        let anchorNode = getAnchorElement(tinymce.activeEditor, node);
        let infoSizeUnit = '';
        let infoSizeValue = '';
        let infoExtension = '';
        ...
        if (anchorNode && anchorNode.title) {
            let extMatch = anchorNode.title.match(/\[(\S+)\s/);
            if (extMatch) infoExtension = extMatch[1];
        }

        {
            let fileInfoElement = anchorNode && anchorNode.nextElementSibling && anchorNode.nextElementSibling.classList.contains('exe-link-data') ? anchorNode.nextElementSibling : null;
            if (fileInfoElement) {
                infoSizeUnit = fileInfoElement.children[0].innerHTML;
                function getSize() {
                    let effectiveDefaultValue = defaultSizeValue;
                    let effectiveDefaultUnit = defaultSizeUnit;
                    if (effectiveDefaultValue === '' && fileInfoElement) {
                        let spanText = fileInfoElement.textContent || '';
                        let match = spanText.match(/(\d+\.?\d*)\s*(B|KB|MB)/);
                        if (match) {
                            effectiveDefaultValue = match[1];
                            effectiveDefaultUnit = match[2];
                        }
                    }
                    if (infoSizeUnit === effectiveDefaultUnit) {
                        infoSizeValue = effectiveDefaultValue;
                    } else {
                        if (effectiveDefaultUnit === 'B') {
                            if (infoSizeUnit === 'KB') {
                                infoSizeValue = parseFloat(effectiveDefaultValue) / 1024;
                            } else if (infoSizeUnit === 'MB') {
                                infoSizeValue = parseFloat(effectiveDefaultValue) / 1024 / 1024;
                            }
                        } else if (effectiveDefaultUnit === 'KB') {
                            if (infoSizeUnit === 'B') {
                                infoSizeValue = parseFloat(effectiveDefaultValue) * 1024;
                            } else if (infoSizeUnit === 'MB') {
                                infoSizeValue = parseFloat(effectiveDefaultValue) / 1024;
                            }
                        } else if (effectiveDefaultUnit === 'MB') {
                            if (infoSizeUnit === 'B') {
                                infoSizeValue = parseFloat(effectiveDefaultValue) * 1024 * 1024;
                        ...
                        const _parsed = parseFloat(infoSizeValue); infoSizeValue = isNaN(_parsed) ? '' : _parsed.toFixed(2);
                    }
                }
                getSize();
            }
        ...
        return {
            ...
            fileInfoExtension: infoExtension !== '' ? infoExtension : (url === '' ? '' : url.split('.').pop()),
    ...
    // Resolves asset:// to blob:// for AJAX fetching (blob URLs are same-origin; asset:// URLs are not)
    function resolveUrlForFetch(url) {
        if (url && url.startsWith('asset://') && top.eXeLearningAssetResolver) {
            return top.eXeLearningAssetResolver.resolve(url);
        }
    ...
    function setExtensionFromUrl(api, url, blobUrl) {
        ...
        if (url.startsWith('blob:')) {
            var assetUrl = resolver.getAssetUrlFromBlob(url);
            filename = assetUrl ? resolver.extractFilenameFromAssetUrl(assetUrl)
                                : resolver.extractFilenameFromBlob(url);
        } else if (url.startsWith('asset://')) {
            filename = resolver.extractFilenameFromAssetUrl(url)
                    || (blobUrl && resolver.extractFilenameFromBlob(blobUrl));
        } else {
            return;
        }
        if (!filename) return;
        ...
        if (ext && ext !== filename) {
            api.setData({ fileInfoExtension: ext });
        }
    }
    // Successful AJAX file-info response: file size and file extension
    function applyFileInfoFromResponse(api, url, fetchUrl, res) {
        var size = res.length;
        if (size === 0) return;
        api.setData({ fileInfoSize: size.toString() });
        tinymce.DOM.setAttribs($("#inputFileSize"), { 'original-size': size.toString() + ' B' });
        setExtensionFromUrl(api, url, fetchUrl);
    }
    function FileInfoState(api) {
        setOpacityFileInfo();
        if (showFileInfo) {
            var currentUnit = api.getData().fileInfoUnit || 'B';
            var url = api.getData().url.value;
            api.setData({
                fileInfoExtension: "",
                fileInfoSize: "",
                fileInfoUnit: currentUnit
            });
            resolveUrlForFetch(url).then(function(fetchUrl) {
                if (!fetchUrl) { api.setData({ fileInfoSize: "" }); return; }
                jQuery.ajax({
                    type: "GET",
                    url: fetchUrl,
                    success: function(res){
                        applyFileInfoFromResponse(api, url, fetchUrl, res);
                        if (currentUnit !== 'B') recalcFileSize(currentUnit, api);
                    },
                    error: function(){
                        api.setData({ fileInfoSize: "" });
                    }
                });
            });
        ...
        return {
            ...
            onChange: function (api, _a) {
                ...
                // data files
                    ...
                    let size = api.getData().url.meta && api.getData().url.meta.size;
                    if (size !== undefined) {
                        // File selected from the file manager: metadata is available.
                        ...
                        // URL typed or pasted: no metadata. Clear and fetch size via AJAX.
                        api.setData({ fileInfoSize: "", fileInfoUnit: "B", fileInfoExtension: "" });
                        let currentUrl = api.getData().url.value;
                        if (currentUrl && currentUrl.trim() !== '') {
                            resolveUrlForFetch(currentUrl).then(function(fetchUrl) {
                                if (!fetchUrl) { api.setData({ fileInfoSize: "" }); return; }
                                jQuery.ajax({
                                    type: "GET",
                                    url: fetchUrl,
                                    success: function (res) {
                                        applyFileInfoFromResponse(api, currentUrl, fetchUrl, res);
                                    },
                                    error: function () {
                                        api.setData({ fileInfoSize: "" });
                                    }
    ...
    function recalcFileSize(unit, api) {
        ...
        if (!size || size.trim() === '') {
            // Fallback: original-size not yet set (e.g. XHR still in progress or new link).
            // Use current fileInfoSize as bytes (default unit is always B).
            let currentSize = api.getData().fileInfoSize;
            if (!currentSize || currentSize.trim() === '') return;
            size = currentSize.trim() + ' B';
        }

        let parts = size.trim().split(' ');
        if (parts.length < 2) return;

        let originalValue = parseFloat(parts[0]);
        let originalUnit = parts[1];

        if (isNaN(originalValue)) return;

        if (unit === originalUnit) {
            api.setData({ fileInfoSize: originalValue.toString() });
        } else {
            let bytes;
            if (originalUnit === 'B') {
                bytes = originalValue;
            } else if (originalUnit === 'KB') {
                bytes = originalValue * 1024;
            } else if (originalUnit === 'MB') {
                bytes = originalValue * 1024 * 1024;
            } else {
                return;
            }
            ...
            const _v = parseFloat(newValue.toFixed(2)); api.setData({ fileInfoSize: isNaN(_v) ? '' : _v.toString() });
        }
    }
    ...
    function getAnchorList() {
        ...
        var list = [
            { text: '---', value: '' },
            { text: _('Link to the .elpx file'), value: 'exe-package:elp' }
        ];
        ...
        var currentPageId = eXeLearning?.app?.project?.structure?.nodeSelected?.getAttribute?.('nav-id');
        var bridge = eXeLearning?.app?.project?._yjsBridge;

        // Add anchors from other components on the same page (same-page links: #anchorName)
        // The DOM scan above only covers the active TinyMCE instance; other components need Yjs scan
        if (currentPageId && bridge?.getPageAnchors) {
            var existingValues = list.map(function(item) { return item.value; });
            bridge.getPageAnchors(currentPageId).forEach(function(anchor) {
                var value = '#' + anchor;
                if (existingValues.indexOf(value) === -1) {
                    list.push({ text: anchor, value: value });
                    existingValues.push(value);
                }
            });
        }

        // Add cross-page anchors: exe-node:pageId#anchorName entries for anchors on other pages
        var crossPageAnchors = bridge?.getAllPageAnchors?.(currentPageId);
        if (crossPageAnchors) {
            crossPageAnchors.forEach(function(p) {
                p.anchors.forEach(function(anchor) {
                    list.push({ text: p.pageName + ' #' + anchor, value: 'exe-node:' + p.pageId + '#' + anchor });
                });
            });
        }
