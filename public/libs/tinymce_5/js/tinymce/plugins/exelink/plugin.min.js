...
(function () {
    ...
    function applyFileInfoFromResponse(api, url, res) {
        var size = res.length;
        if (size === 0) return;
        api.setData({ fileInfoSize: size.toString() });
        tinymce.DOM.setAttribs($("#inputFileSize"), { 'original-size': size.toString() + ' B' });
        setExtensionFromBlobUrl(api, url);
    }
    // Get file extension
    function setExtensionFromBlobUrl(api, blobUrl) {
        if (!blobUrl || !blobUrl.startsWith('blob:') || !top.eXeLearningAssetResolver) return;
        var assetUrl = top.eXeLearningAssetResolver.getAssetUrlFromBlob(blobUrl);
        if (!assetUrl) return;
        var filename = top.eXeLearningAssetResolver.extractFilenameFromAssetUrl(assetUrl);
        if (!filename) return;
    ...
    function FileInfoState(api) {
        ...
        if (showFileInfo) {
            ...
            var xhr = jQuery.ajax({
                type: "GET",
                url: api.getData().url.value,
                success: function(res){
                    applyFileInfoFromResponse(api, api.getData().url.value, res);
                    ...
                    if (size !== undefined) {
                        ...
                        if (currentUrl && currentUrl.trim() !== '') {
                            jQuery.ajax({
                                ...
                                url: currentUrl,
                                success: function (res) {
                                    applyFileInfoFromResponse(api, currentUrl, res);
                                },
                                error: function () {
                                    api.setData({ fileInfoSize: "" });
                                }
