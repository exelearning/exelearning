      ...
      return {
        ...
        onAction: function (api, details) {
          if (details.name == 'imageOptimizer') {
            ...
            const filemanager = window.eXeLearning && window.eXeLearning.app && window.eXeLearning.app.modals && window.eXeLearning.app.modals.filemanager;
            if (filemanager) {
              filemanager.show({
                accept: 'image',
                onSelect: function(result) {
                  var currentData = api.getData();
                  // Use blobUrl for immediate preview and saving
                  // The generic TinyMCE settings will handle saving this to AssetManager if needed
                  // but here we are in a custom dialog.
                  // exeimage plugin handles the data.
                  
                  var newData = {
                    src: {
                      value: result.blobUrl,
                      meta: currentData.src.meta || {}
                    }
                  };
                  
                  // Auto-fill alt text if empty and filename available
                  if ((!currentData.alt || currentData.alt === '') && result.asset && result.asset.filename) {
                     newData.alt = result.asset.filename;
                  }
                  
                  api.setData(newData);
                  
                  // Recalculate dimensions and update other fields
                  changeSrc(helpers, info, state, api);
                }
              });
            } else {
              console.error('File Manager not available');
              tinymce.activeEditor.windowManager.alert(_('File Manager not available'));
            }
          }
        },
