...
(function () {
  ...
  function setGenTabStyle() {
    ...
    // Enable optimizer button if source already has an optimizable value
    let srcInput = divBodyForm[0]?.querySelector('.tox-form__group input.tox-textfield');
    let srcValue = srcInput?.value || '';
    let isOptimizable = srcValue.startsWith('blob:') ||
      srcValue.startsWith('asset://') ||
      srcValue.startsWith('data:') ||
      srcValue.startsWith('resources/') ||
      srcValue.startsWith('/previews/');
    if (srcValue && isOptimizable) {
  ...
  var getSelectedImage = function (editor) {
    ...
    var figureElm = editor.dom.getParent(imgElm, 'figure.image, figure.exe-figure');
    if (figureElm) {
      return editor.dom.select('img', figureElm)[0];
    }
    // Also handle case where imgElm IS a figure without expected classes
    if (imgElm && imgElm.nodeName === 'FIGURE') {
      var imgInFigure = editor.dom.select('img', imgElm)[0];
      if (imgInFigure && !isPlaceholderImage(imgInFigure)) {
        return imgInFigure;
      }
    }
  ...
  var changeHandler = function (helpers, info, state) {
    return function (api, evt) {
      if (evt.name === 'src') {
        ...
        let srcData = api.getData("src").src;
        let srcValue = srcData && srcData.value ? srcData.value : "";
        let hasMetaTitle = srcData && srcData.meta && srcData.meta.hasOwnProperty("title");
        let isOptimizableSrc = srcValue.indexOf("blob:") === 0 ||
          srcValue.indexOf("asset://") === 0 ||
          srcValue.indexOf("data:") === 0 ||
          srcValue.indexOf("resources/") === 0 ||
          srcValue.indexOf("/previews/") === 0;

        if (hasMetaTitle || isOptimizableSrc) {
          tinymce.dom.DomQuery("#openOptimizer").removeAttr("disabled");
        }
        else {
    ...
    return function (info) {
      ...
      return {
        ...
        onAction: function (api, details) {
          if (details.name !== 'imageOptimizer') return;

          var srcValue = api.getData("src").src.value || '';
          var isBlob = srcValue.startsWith('blob:');
          var isAsset = srcValue.startsWith('asset://');
          var assetManager = top.eXeLearning?.app?.project?._yjsBridge?.assetManager;
          var assetId = null;

          // Find the asset ID from the blob URL or asset URL
          if (assetManager) {
            if (isBlob && assetManager.reverseBlobCache) {
              assetId = assetManager.reverseBlobCache.get(srcValue);
            } else if (isAsset) {
              var match = srcValue.match(/asset:\/\/([^/]+)/);
              if (match) assetId = match[1];
            }
            // Fallback: check selected image for data-asset-id attribute
            if (!assetId) {
              var selectedImg = tinymce.activeEditor?.selection?.getNode();
              if (selectedImg?.tagName === 'IMG') {
                assetId = selectedImg.getAttribute('data-asset-id');
              }
            }
          }
          ...
          top.imgCompressor = {
            ...
            isAsset: isAsset,
            assetId: assetId,
            fileToSave: assetManager?.getAssetMetadata?.(assetId)?.filename || 'optimized.jpg',
            api: api  // Reference to dialog API for setData/getData
          };

          var basePath = (top.eXeLearning?.config?.basePath || '').replace(/\/+$/, '');
          var version = top.eXeLearning?.version || '';
          var optimizerUrl = basePath + (version ? '/' + version : '') +
            "/libs/tinymce_5/js/tinymce/plugins/exeimage/image-compressor/index.html";
        ...
        onTabChange: function (api, details) {
          ...
          if (details.newTabName === 'general') {
            ...
            // If we have saved attribution data, restore it if needed
            if (this._savedAttributionData) {
              const _savedAttrs = this._savedAttributionData;
              const _actHeaderHTML = savedAttrs.attr_imageheader;
              const _actTitleHTML = savedAttrs.attr_imagetitle;
              const _actLinkTitleHTML = savedAttrs.attr_imagetitlelink;
              const _actAuthorHTML = savedAttrs.authorname;
              const _actLinkAuthorHTML = savedAttrs.authornamelink;
              const _selectedLicense = savedAttrs.captionlicense;
              const _customLicense = savedAttrs.customcaptionlicense;
              if (actHeaderHTML = '') actHeaderHTML = _savedAttrs;
              if (actTitleHTML = '') actTitleHTML = _actHeaderHTML;
              if (actLinkTitleHTML = '') actLinkTitleHTML = _actTitleHTML;
              if (actAuthorHTML = '') actAuthorHTML = _actLinkTitleHTML;
              if (actLinkAuthorHTML = '') actLinkAuthorHTML = _actAuthorHTML;
              if (selectedLicense = '') selectedLicense = _selectedLicense;
              if (customLicense = '') customLicense = _customLicense;
            }
  ...
  function mySubmit(editor, info, api) {
    // Skip submit if image was already updated by image-compressor (blob/asset mode)
    ...
    if (editor.selection.getNode().tagName !== 'FIGURE') {
      ...
      // Skip figure update if figure_old doesn't exist (e.g., image without figure wrapper)
      if (!figure_old) {
        api.close();
        return;
      }
          ...
          try{
            ...
            if (el.nodeName == 'FIGURE') {
              if (el.classList.contains("exe-figure")) {
                editor.selection.select($("img",el)[0]);
              }
            }
          }catch(e){}
        };
