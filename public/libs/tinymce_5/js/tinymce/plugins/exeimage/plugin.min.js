  ...
  var fromImageData = function (image) {
    ...
    if (image.alignstyle) {
      ...
      if (figure != null) {
        ...
        // Determinar alineación por defecto según el contexto
        if (isInColumn) {
          actualAlign = '';
        } else {
          // Verificar si el cursor está dentro de un párrafo con texto
          let cursorNode = tinymce.activeEditor.selection.getNode();
          let inParagraph = tinymce.activeEditor.dom.getParent(cursorNode, 'p');
          
          if (inParagraph && (inParagraph.textContent.trim().length > 0 || inParagraph.childNodes.length > 1)) {
            // Hay texto en el párrafo, usar baseline para imagen inline
            actualAlign = 'baseline';
            img_v_style = 'baseline';
          } else {
            // Párrafo vacío o nuevo, usar center por defecto
    ...
    return {
      ...
      alignstyle: image.alignstyle || actualAlign || 'center'
    };
  };
  ...
  function mySubmit(editor, info, api) {
    ...
    if (!hasAttribution && !isEditingExistingFigure) {
      ...
      if (alignStyle) {
        setTimeout(function() {
          ...
          if (imgNode && imgNode.nodeName === 'IMG') {
            ...
            currentStyle += ' ' + alignStyle;
            
            imgNode.setAttribute('style', currentStyle.trim());
          }
        }, 10);
      }
      ...
      if (currentImage) {
        ...
        currentImage.setAttribute('style', currentStyle.trim());
      ...
      setTimeout(function() {
        ...
        if (imgNode && imgNode.nodeName === 'IMG') {
          ...
          if (alignStyle) {
            if (figureStyle && !figureStyle.endsWith(';')) {
              figureStyle += ';';
            }
            figureStyle += ' ' + alignStyle;
          }
          
          let y = editor.dom.create('figure', { 'class': 'exe-figure', 'style': figureStyle.trim() }, custom_div_html);
    ...
    editor.on('BeforeExecCommand', function(e) {
      if (e.command === 'JustifyLeft' || e.command === 'JustifyCenter' || 
        ...
        if (figure) {
          ...
          if (imgs.length > 0) {
            var imgStyle = imgs[0].getAttribute('style') || '';
            imgStyle = imgStyle.replace(/float\s*:\s*(left|right|none)\s*;?/gi, '');
            imgStyle = imgStyle.replace(/margin-left\s*:\s*(auto|1\.5em)\s*;?/gi, '');
            imgStyle = imgStyle.replace(/margin-right\s*:\s*(auto|1\.5em)\s*;?/gi, '');
            imgStyle = imgStyle.trim();
            if (imgStyle) {
              imgs[0].setAttribute('style', imgStyle);
            } else {
        ...
        currentStyle = currentStyle.replace(/float\s*:\s*(left|right|none)\s*;?/gi, '');
        currentStyle = currentStyle.replace(/display\s*:\s*(block|inline-block)\s*;?/gi, '');
        currentStyle = currentStyle.replace(/margin-left\s*:\s*auto\s*;?/gi, '');
        currentStyle = currentStyle.replace(/margin-right\s*:\s*auto\s*;?/gi, '');
        currentStyle = currentStyle.replace(/margin-left\s*:\s*1\.5em\s*;?/gi, '');
        currentStyle = currentStyle.replace(/margin-right\s*:\s*1\.5em\s*;?/gi, '');
        ...
        if (e.command === 'JustifyLeft') {
          ...
          currentStyle += ' float: left; margin-right: 1.5em;';
        } else if (e.command === 'JustifyRight') {
          if (currentStyle && !currentStyle.endsWith(';')) {
            currentStyle += ';';
          }
          currentStyle += ' float: right; margin-left: 1.5em;';
        } else if (e.command === 'JustifyCenter' || e.command === 'JustifyFull') {
          if (currentStyle && !currentStyle.endsWith(';')) {
            currentStyle += ';';
          }
          currentStyle += ' display: block; margin-left: auto; margin-right: auto;';
        }
        
        if (currentStyle) {
          targetElement.setAttribute('style', currentStyle.trim());
          editor.dom.setAttrib(targetElement, 'style', currentStyle.trim());
        } else {
