...
(function () {
    ...
    function GetActualData(editor) {
        ...
        actualContainer = editor.dom.getParent(actualMedia, 'figure.exe-figure');
        if (!actualContainer) {
          actualContainer = editor.dom.getParent(actualMedia, 'figure');
        }
        ...
        if (actualContainer != null&&actualContainer.nodeName=="FIGURE") {
            ...
            if (actualContainer.className.indexOf("exe-figure exe-media")==0) {
              ...
              // For simple media (span without figure): read alignment from span inline styles
              var spanStyle = actualMedia.getAttribute('style') || '';
              var hasFloat = spanStyle.indexOf('float') !== -1;
              var hasMarginAuto = spanStyle.indexOf('margin-left: auto') !== -1 && spanStyle.indexOf('margin-right: auto') !== -1;
              var hasVerticalAlign = spanStyle.indexOf('vertical-align') !== -1;
              
              // Build actualFigureClass for simple media (without exe-figure exe-media prefix)
              actualFigureClass = '';
              
              if (hasFloat && spanStyle.indexOf('float: left') !== -1) {
                actualFigureClass += ' position-left';
              } else if (hasFloat && spanStyle.indexOf('float: right') !== -1) {
                actualFigureClass += ' position-right';
              } else if (hasMarginAuto) {
                actualFigureClass += ' position-center';
              } else if (hasVerticalAlign) {
                var valignMatch = spanStyle.match(/vertical-align\s*:\s*(baseline|top|middle|bottom|text-top|text-bottom)/i);
                if (valignMatch) {
                  actualFigureClass += ' vertical-align-' + valignMatch[1];
                }
              }
    ...
    var handleInsert = function (editor, html) {
        ...
        var figure = editor.dom.getParent(elm, 'figure.exe-figure');
        if (!figure) {
          figure = editor.dom.getParent(elm, 'figure');
        }
        if (figure) {
        ...
        setTimeout(function() {
          var figures = editor.dom.select('figure.exe-figure');
          if (figures.length > 0) {
    ...
    var getCustomHTML = function(html){
      ...
      if (mediaHeader != '' || authorName != '' || authorNameLink != '' || mediaTitle != '' || mediaTitleLink != '' || captionLicense != '') {
        ...
        if (actualFigureClass!="") {
          ...
          // Check for vertical-align types
          var hasVerticalAlign = actualFigureClass.indexOf('vertical-align-') !== -1;
          var verticalAlignValue = '';
          if (hasVerticalAlign) {
            var valignMatch = actualFigureClass.match(/vertical-align-(baseline|top|middle|bottom|text-top|text-bottom)/);
            if (valignMatch) {
              verticalAlignValue = valignMatch[1];
            }
          }
          
          if (isInColumn) {
            // In columns: use float for left/right, nothing for center
            if (hasLeft) {
              figureStyle = "float: left; margin-right: 1.5em;";
            } else if (hasRight) {
              figureStyle = "float: right; margin-left: 1.5em;";
            } else if (hasVerticalAlign && verticalAlignValue) {
              figureStyle = "display: inline-block; vertical-align: " + verticalAlignValue + ";";
            }
          } else {
            // Outside columns: use float for left/right, margin auto for center
            if (hasLeft) {
              figureStyle = "float: left; margin-right: 1.5em;";
            } else if (hasRight) {
              figureStyle = "float: right; margin-left: 1.5em;";
          ...
          // Default alignment: center outside columns, nothing in columns
          if (!isInColumn) {
            figureStyle = "display: block; margin-left: auto; margin-right: auto;";
          }
        }
        ...
        // Combine width with alignment styles
        var extraStyle = "width:" + mediaWidth + "px;";
        if (figureStyle !== "") {
          extraStyle += " " + figureStyle;
        }
        ...
        // Simple media (span without figure): apply alignment styles to the span
        var spanStyle = "";
        
        if (actualFigureClass!="") {
          var hasLeft = actualFigureClass.indexOf('position-left') !== -1;
          var hasRight = actualFigureClass.indexOf('position-right') !== -1;
          var hasCenter = actualFigureClass.indexOf('position-center') !== -1;
          var hasVerticalAlign = actualFigureClass.indexOf('vertical-align-') !== -1;
          var verticalAlignValue = '';
          if (hasVerticalAlign) {
          ...
          if (isInColumn) {
            // In columns: use float for left/right, nothing for center
            if (hasLeft) {
              spanStyle = "float: left; margin-right: 1.5em;";
            } else if (hasRight) {
              spanStyle = "float: right; margin-left: 1.5em;";
            } else if (hasVerticalAlign && verticalAlignValue) {
              spanStyle = "display: inline-block; vertical-align: " + verticalAlignValue + ";";
            }
          } else {
            // Outside columns: use float for left/right, margin auto for center
            if (hasLeft) {
              ...
              spanStyle = "display: inline-block; margin-left: auto; margin-right: auto;";
            } else if (hasVerticalAlign && verticalAlignValue) {
              spanStyle = "display: inline-block; vertical-align: " + verticalAlignValue + ";";
            }
          }
        ...
        // Apply alignment styles to span if needed
        if (spanStyle !== "") {
          tmpDiv.html(html);
          var span = tmpDiv.children('span[data-mce-object]').first();
          if (span.length > 0) {
            var existingStyle = span.attr('style') || '';
            // Clean existing alignment styles
            existingStyle = existingStyle.replace(/float\s*:\s*(left|right)\s*;?\s*/gi, '');
            existingStyle = existingStyle.replace(/margin-(left|right)\s*:\s*[^;]+;?\s*/gi, '');
            existingStyle = existingStyle.replace(/display\s*:\s*(block|inline-block)\s*;?\s*/gi, '');
            existingStyle = existingStyle.replace(/vertical-align\s*:\s*[^;]+;?\s*/gi, '');
            // Apply new alignment
            span.attr('style', (existingStyle + ' ' + spanStyle).trim());
            res = tmpDiv.html();
          } else {
            res = html;
          }
        ...
        var showDialog$1 = function () {
            ...
            if (figure.length==1 && elm.nodeName!="IMG" && !isIframe) {
              ...
              if (figureHTML.indexOf('<iframe')!=-1 || figureHTML.indexOf('<video')!=-1 || figureHTML.indexOf('<audio')!=-1 || figureHTML.indexOf('<img data-mc')!=-1) {
                var mediaElement = figure[0].querySelector('video, audio, iframe, embed, span[data-mce-object]');
                if (mediaElement) {
    ...
    var setup = function (editor) {
        ...
        editor.on('click', function(e) {
            ...
            if (target.nodeName === 'FIGURE' && editor.dom.hasClass(target, 'exe-figure')) {
                var mediaElement = editor.dom.select('video, audio, iframe, embed, span[data-mce-object]', target)[0];
                if (mediaElement) {
                    editor.selection.select(mediaElement);
                    e.preventDefault();
                    e.stopPropagation();
        ...
        editor.on('BeforeExecCommand', function(e) {
            ...
            if (cmd === 'JustifyLeft' || cmd === 'JustifyRight' || cmd === 'JustifyCenter' || cmd === 'JustifyFull') {
                ...
                var figure = editor.dom.getParent(node, 'figure.exe-figure', editor.getBody());
                ...
                if (figure) {
                    ...
                    mediaElement = figure.querySelector('video, audio, iframe, embed, span[data-mce-object]');
                } else {
                    if (node.nodeName === 'VIDEO' || node.nodeName === 'AUDIO' || node.nodeName === 'IFRAME' || node.nodeName === 'EMBED') {
                        ...
                        var parent = editor.dom.getParent(node, 'span[data-mce-object],video,audio,iframe,embed');
                        if (parent) {
                            mediaElement = parent;
                        }
                    }
                    ...
                    if (mediaElement) {
                        ...
                        parentBlock = editor.dom.getParent(targetElement, 'p,div', editor.getBody());
                    }
                }
                ...
                if (figure && mediaElement) {
                    var mediaStyle = mediaElement.getAttribute('style') || '';
                    mediaStyle = mediaStyle.replace(/float\s*:\s*(left|right|none)\s*;?/gi, '');
                    mediaStyle = mediaStyle.replace(/margin-left\s*:\s*1\.5em\s*;?/gi, '');
                    mediaStyle = mediaStyle.replace(/margin-right\s*:\s*1\.5em\s*;?/gi, '');
                    mediaStyle = mediaStyle.trim();
                    if (mediaStyle) {
                        mediaElement.setAttribute('style', mediaStyle);
                    } else {
                ...
                if (parentBlock) {
                    var blockStyle = parentBlock.getAttribute('style') || '';
                    blockStyle = blockStyle.replace(/text-align\s*:\s*[^;]+\s*;?/gi, '');
                    blockStyle = blockStyle.trim();
                    
                    if (cmd === 'JustifyCenter' || cmd === 'JustifyFull') {
                        if (blockStyle && !blockStyle.endsWith(';')) {
                            blockStyle += ';';
                        }
                    ...
                    if (blockStyle) {
                        parentBlock.setAttribute('style', blockStyle.trim());
                    } else {
                ...
                if (cmd === 'JustifyLeft') {
                    ...
                    // Use block for figures with exe-media class, inline-block for simple media
                    var displayValue = (figure && targetElement.classList.contains('exe-media')) ? 'block' : 'inline-block';
                    currentStyle += ' display: ' + displayValue + '; margin-left: auto; margin-right: auto;';
                }
