...
jQuery(document).ready(function ($) {
	...
	tinymce.PluginManager.add('codemagic', function (editor, url) {
		...
		function open_codemagic() {
			...
			// Use capabilities first, fall back to __EXE_STATIC_MODE__ for early init
			var capabilities = window.eXeLearning && window.eXeLearning.app && window.eXeLearning.app.capabilities;
			var isStaticMode = capabilities
				? !capabilities.storage.remote
				: window.__EXE_STATIC_MODE__;

			// In static mode, use the direct file path (no API server available)
			// In server mode, use API endpoint to bypass Bun's HTML bundler
			if (isStaticMode) {
				codemagicUrl = basePath + '/libs/tinymce_5/js/tinymce/plugins/codemagic/codemagic.html';
			} else {
