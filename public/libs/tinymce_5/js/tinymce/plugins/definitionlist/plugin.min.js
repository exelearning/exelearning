...
tinymce.PluginManager.add('definitionlist', function(editor, url) {
	...
	function addListOrTerm() {
			...
			var term = "<dt>"+_('Word')+"</dt>\n<dd><p>"+_('Definition')+"</p></dd>\n";
			
			// No selection and not in DL
			var p = editor.dom.getParent(se.getNode(), 'DL');
			if (se.isCollapsed() && !p) {
				editor.windowManager.confirm(
					_('A new Definition List will be created. To add a new term, just place the cursor at the end of a definition and click again the Definition List Button.'), 
					function(s){
						if (s) {
				...
				// Stale selection outside any DL: nothing to move the cursor into (#2273)
				if (!p) return;
