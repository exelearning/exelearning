...
export default class App {
    ...
    registerPreviewServiceWorker() {
        ...
        this._previewSwRegistrationPromise = (async () => {
            try {
                // Unregister any existing SW to ensure we use the latest version
                // This is important for Electron where the app restarts frequently
                const existingRegistration = await navigator.serviceWorker.getRegistration(basePath);
                if (existingRegistration) {
                    console.log('[Preview SW] Unregistering existing SW for fresh start');
                ...
                console.log('[Preview SW] Registering:', swPath);
                ...
                console.log('[Preview SW] Ready and active');
                return registration;
            } catch (error) {
                console.error('[Preview SW] Registration failed:', error);
                return null;
    ...
    async sendContentToPreviewSW(files, options = {}) {
        ...
        if (this._previewSwRegistrationPromise) {
            console.log('[Preview SW] Waiting for registration promise...');
            await this._previewSwRegistrationPromise;
            console.log('[Preview SW] Registration promise resolved');
        }

        const sw = this.getPreviewServiceWorker();
        console.log('[Preview SW] Got SW:', sw ? 'yes' : 'no', sw?.state);
        if (!sw) {
            throw new Error('Preview Service Worker not available');
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            messageChannel.port1.onmessage = (event) => {
                console.log('[Preview SW] Received message on port:', event.data?.type);
                if (event.data?.type === 'CONTENT_READY') {
                    // Content received by SW, now verify it can actually serve requests
                    // This extra verification step handles Firefox's stricter event timing
                    // Create a new channel for the verification since port2 was already transferred
                    ...
                    verifyChannel.port1.onmessage = (verifyEvent) => {
                        ...
                        if (verifyEvent.data?.ready) {
                            ...
                            reject(new Error('SW content not ready after verification'));
                        }
                    };
                    sw.postMessage({ type: 'VERIFY_READY' }, [verifyChannel.port2]);
                } else if (event.data?.type === 'READY_VERIFIED') {
                    // Direct response (when SW is controller)
                    clearTimeout(timeoutId);
                    messageChannel.port1.close();
                    if (event.data.ready) {
                        resolve({ fileCount: event.data.fileCount });
            ...
            console.log('[Preview SW] Sending SET_CONTENT with', Object.keys(files).length, 'files');
            sw.postMessage(
                {
                    type: 'SET_CONTENT',
                    data: { files, options },
            ...
            console.log('[Preview SW] Message sent, waiting for response...');
