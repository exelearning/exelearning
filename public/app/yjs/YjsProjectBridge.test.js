...
describe('YjsProjectBridge', () => {
  ...
  describe('updateRemoteBlock', () => {
    ...
    it('updates block icon from structured icon data for remote collaborators', async () => {
      ...
      await bridge.updateRemoteBlock(
        { id: 'block-1', iconName: 'bi-alarm', icon: { source: 'bootstrap', value: 'alarm' } },
        'page-1'
      );

      expect(mockBlockNode.icon).toEqual({ source: 'bootstrap', value: 'alarm' });
      expect(mockBlockNode.iconName).toBe('bi-alarm');
  ...
  describe('_syncBlockIcon', () => {
    ...
    it('renders bootstrap icons with the shared mask runtime', () => {
      const mockIconEl = {
        innerHTML: '',
        style: {
          removeProperty: mock(() => {}),
      ...
      bridge._syncBlockIcon(mockIconEl, { source: 'bootstrap', value: 'alarm' }, 'block-1');

      expect(mockIconEl.innerHTML).toContain('class="exe-bootstrap-icon"');
