...
class YjsStructureBinding {
            ...
            this.manager.getDoc().transact(() => {
              Object.entries(updates).forEach(([key, value]) => {
                if (key === 'properties' && typeof value === 'object') {
                  ...
                  const legacy = value?.source === 'bootstrap' ? `bi-${value.value}` : (value?.value || '');
                  blockMap.set('iconName', legacy);
                } else if (key === 'iconName') {
                  blockMap.set('iconName', value);
                  if (!blockMap.get('icon')) {
                    const isBootstrap = typeof value === 'string' && value.startsWith('bi-');
                    const isAsset = typeof value === 'string' && (value.startsWith('asset://') || value.startsWith('/'));
                    blockMap.set('icon', isBootstrap
                      ? { source: 'bootstrap', value: value.replace(/^bi-/, '') }
                      : (isAsset
                        ? { source: 'asset', value }
  ...
  createBlockMapFromApi(apiBlock) {
    ...
    blockMap.set('icon', importedIconName ? (importedIconName.startsWith('bi-') ? { source: 'bootstrap', value: importedIconName.replace(/^bi-/, '') } : { source: 'theme', value: importedIconName }) : { source: 'none', value: '' });
