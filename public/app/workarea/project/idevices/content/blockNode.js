...
import { BOOTSTRAP_ICON_CATALOG } from './bootstrapIconCatalog.js';
...
const LEGACY_ICON_MAP = {
    ...
    activity: 'list-check',
    read: 'book-fill',
    reflection: 'chat-left-quote-fill',
    objectives: 'bullseye',
    keypoints: 'bookmark-star-fill',
};
...
const localBlockIconRuntime = {
    ...
    getBootstrapIconPath(iconName, options = {}) {
        ...
        const safeIconName = !iconName
            ...
            : (!Array.isArray(catalog) || catalog.includes(iconName))
                ? iconName
                : 'question-circle';
        return this.resolveAppAssetUrl(`/libs/bootstrap-icons/icons/${safeIconName}.svg`, options);
    },

    renderBootstrapMaskIcon(iconName, options = {}) {
        const iconPath = this.getBootstrapIconPath(iconName, options);
...
export default class IdeviceBlockNode {
    ...
    normalizeIconDescriptor(iconData, legacyIconName = '') {
        ...
        if (legacy.startsWith('bi-')) {
            const value = legacy.replace(/^bi-/, '');
            return { source: 'bootstrap', value, name: value };
        }
        ...
        if (LEGACY_ICON_MAP[legacy]) {
            ...
            return { source: 'bootstrap', value: mapped, name: mapped };
        }
...
<path d="M28.5 11.5L11.5 28.5" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"/>
    ...
    getBootstrapIconPath(iconName) {
        return blockIconRuntime.getBootstrapIconPath(iconName, {
    ...
    getBootstrapSpritePath() {
        return this.resolveAppAssetUrl('/libs/bootstrap-icons/bootstrap-icons.svg');
    }
    ...
    renderBootstrapMaskIcon(iconName) {
        return blockIconRuntime.renderBootstrapMaskIcon(iconName, {
            app: window.eXeLearning?.app,
            config: window.eXeLearning?.config,
            catalog: BOOTSTRAP_ICON_CATALOG,
        });
    ...
    renderBootstrapSpriteIcon(iconName) {
        const safeIconName = BOOTSTRAP_ICON_CATALOG.includes(iconName) ? iconName : 'question-circle';
        const spritePath = this.getBootstrapSpritePath();
        const spriteHref = `${spritePath}#${safeIconName}`;
        return `<svg class="exe-bootstrap-icon-sprite" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true">
<use href="${spriteHref}" xlink:href="${spriteHref}"></use>
    ...
    renderIconPreviewHtml(iconDescriptor) {
        ...
        if (icon.source === 'bootstrap') {
            return this.renderBootstrapMaskIcon(icon.value);
        }
...
<path d="M20 12V28M12 20H28" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
    ...
    createModalIconOption(iconConfig, title, currentIcon, options = {}) {
        ...
        const modalPreviewHtml =
            iconConfig.source === 'bootstrap'
                ? this.renderBootstrapSpriteIcon(iconConfig.value)
    ...
    filterModalBootstrapIcons(iconsElements, query) {
        ...
        iconsElements.forEach((icon) => {
            ...
            icon.style.display = !normalizedQuery || val.includes(normalizedQuery) ? '' : 'none';
        });
    }
    ...
    makeModalChangeIconBody() {
        ...
        // Show filled icons first, then regular icons that have no fill variant
        const fillIcons = BOOTSTRAP_ICON_CATALOG.filter(n => n.endsWith('-fill'));
        const fillSet = new Set(fillIcons);
        const orderedIcons = [
            ...fillIcons,
            ...BOOTSTRAP_ICON_CATALOG.filter(n => !n.endsWith('-fill') && !fillSet.has(`${n}-fill`)),
        ];
        for (const iconName of orderedIcons) {
            const option = appendOption({ source: 'bootstrap', value: iconName, name: iconName }, iconName);
            option.setAttribute('icon-id', `bi-${iconName}`);
        }
    ...
    addBehaviourToModalChangeIconBody() {
        ...
        if (searchInput) {
            searchInput.addEventListener('input', () => {
                this.filterModalBootstrapIcons(iconsElements, searchInput.value);
            });
        }
    ...
    apiUpdateIcon(icon) {
        ...
        this.iconName = normalized.source === 'bootstrap' ? `bi-${normalized.value}` : (normalized.value || '');
