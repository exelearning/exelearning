...
describe('ModalOdeBrokenLinks', () => {
  ...
  beforeEach(() => {
    ...
    window.eXeLearning = {
      app: {
        ...
        api: {
          getOdeSessionBrokenLinks: vi.fn().mockResolvedValue({ brokenLinks: [] }),
          app: {
            menus: {
              navbar: {
                utilities: {
  ...
  describe('makeTbodyElements', () => {
    it('should create table rows from broken links data', () => {
      const data = {
        brokenLinks: [
          {
            ...
            nTimesBrokenLinks: 1,
            pageNamesBrokenLinks: 'Home',
            blockNamesBrokenLinks: 'Content',
      ...
      expect(tbody.querySelector('td').textContent).toBe('http://bad.link');
    });
  });
  ...
  describe('show', () => {
    it('should set title and body content', async () => {
      vi.useFakeTimers();
      modal.show({ brokenLinks: [] });
      vi.advanceTimersByTime(500);
      expect(mockElement.querySelector('.modal-title').innerHTML).toBe('Broken Links');
  ...
  describe('downloadCsv', () => {
    it('should call api and trigger download', async () => {
      ...
      expect(window.eXeLearning.app.api.getOdeSessionBrokenLinks).toHaveBeenCalled();
