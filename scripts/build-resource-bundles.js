...
const ELECTRONICS_LOGIC_PATH = path.join(IDEVICES_BASE_PATH, 'electronics-logic');
const ELECTRONICS_LOGIC_GRADER_BUNDLE = path.join(
    ELECTRONICS_LOGIC_PATH,
    'export',
...
/**
 * Bundle the canonical CommonJS Boolean Core and grader for offline browsers.
 */
async function buildElectronicsLogicGrader() {
    const entrypoint = path.join(ELECTRONICS_LOGIC_PATH, 'core', 'boolean-grader-browser.mjs');
    if (!fs.existsSync(entrypoint)) return;

    console.log('\nBuilding Electronics Logic grader...');
    const result = await Bun.build({
        entrypoints: [entrypoint],
        outdir: path.dirname(ELECTRONICS_LOGIC_GRADER_BUNDLE),
        naming: path.basename(ELECTRONICS_LOGIC_GRADER_BUNDLE),
        target: 'browser',
        format: 'iife',
        minify: true,
    });
    if (!result.success || !fs.existsSync(ELECTRONICS_LOGIC_GRADER_BUNDLE)) {
        const details = result.logs.map(log => log.message).join('\n');
        throw new Error(`Could not build Electronics Logic grader bundle.\n${details}`);
    }
    const size = fs.statSync(ELECTRONICS_LOGIC_GRADER_BUNDLE).size;
    console.log(`  electronics-logic-grader.bundle.js: ${(size / 1024).toFixed(1)} KB`);
}
...
function buildThemeBundles(manifest) {
    ...
    const themes = fs
        .readdirSync(THEMES_BASE_PATH, { withFileTypes: true })
...
function buildIdevicesBundle(manifest) {
    ...
    const idevices = fs
        .readdirSync(IDEVICES_BASE_PATH, { withFileTypes: true })
        .filter(d => d.isDirectory() && !d.name.startsWith('.'))
        .map(d => d.name);
    ...
    console.log(
        `  ${Object.keys(manifest.idevices).length} iDevices, ${allFiles.length} files, ${(zipBuffer.length / 1024).toFixed(1)} KB`,
    );
}
...
function buildContentCssBundle(manifest) {
    ...
    const scannedFiles = scanDirectory(cssPath).filter(f => f.relativePath.endsWith('.css'));
...
async function build() {
    ...
    await buildElectronicsLogicGrader();
